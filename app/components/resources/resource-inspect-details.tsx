...
const TENANT_QUERY = `
  query ResourceInspectTenant($id: ID!) {
    tenant(id: $id) { id name }
  }
`;

const ENTITY_QUERY = `
  query ResourceInspectOwner($id: ID!) {
...
export function ResourceInspectDetails({ row }: { row: Row | null }) {
  ...
  const tenantQuery = useQuery({
    ...
    queryKey: ["resource-inspect-tenant", tenantId],
    queryFn: ({ signal }) =>
  ...
  const ownerQuery = useQuery({
    ...
    queryKey: ["resource-inspect-owner", ownerId],
    queryFn: ({ signal }) =>
  ...
  const ownerName = ownerQuery.data?.entity.name ?? ownerId;
