...
This is an Atom action check over permission blocks. `execute` and `manage` are
global action names; the endpoint ID or tenant scope decides where those actions
apply for this custom endpoint execution.
...
| 403 on execution | Caller lacks `execute` or `manage` access | Grant `execute` on the endpoint object or its tenant scope |
