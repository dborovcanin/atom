...
- **Two channels — DB audit vs stdout observability.** `audit_logs` (the persisted compliance trail) is written by `audit::write()` / `write_hot_path()`, fire-and-forget — it logs failures but never propagates them to the caller. That set is unchanged: auth (`auth.login`/`signup`/`credential_authenticate`), authz (`authz.check`/`explain`), `entity`/`resource` update+delete, the `*.restore`/`*.purge` admin ops, certs, credentials, `action_assignment_rule`. **Observability is a separate, stdout/stderr-only channel** — no DB writes: `audit::write()` now also emits a structured tracing line per row (`log_audit_event`, level keyed to outcome, `details` included), and operations that are *not* part of the DB trail call `audit::observe_result(meta, details, &result)` which logs only (never inserts). `observe_result` classifies `Ok` → info `allow`, `Err` → `AppError::audit_outcome()` (`Unauthorized`/`Forbidden` → warn `deny`, else → error, error string folded into `details`). So **create** mutations and the non-audited **update/delete** paths (group/role/tenant/profile/api_endpoint/permission_block/role_assignment/direct_policy/action) are observable in logs (success **and** failure) without touching the DB. Rule: never call both `write` and `observe_result` for one operation — `write` already logs. Forward-compat for request-id correlation (option D): extend `AuditMeta` with `request_id: Option<String>` — the single insertion point.
